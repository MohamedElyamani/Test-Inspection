...
import { InspectionRequest } from '../interface/inspction-service-order';
import { InspctionServiceOrderService } from '../service/inspction-service-order.service';
import { SharedModule } from "src/app/_metronic/shared/shared.module";

@Component({
  selector: 'app-inspction-service-order-list',
  templateUrl: './inspction-service-order-list.component.html',
  styleUrls: ['./inspction-service-order-list.component.css'],
 
 
})
export class InspctionServiceOrderListComponent implements OnInit {

  dataSource: InspectionRequest[] = []; // Array to hold inspection request data
   ...
   constructor(private _inspectionReq: InspctionServiceOrderService, 
   ...
   ngOnInit() {
     this.inspectionReqOrderList();
   }
 inspectionReqOrderList() {
   this._inspectionReq.getInspectionReqOrderList().subscribe({
    next: (res) => {
 ...
 navigateToEdit(row: any) {
   ...
   this.selectedRow = rows;
    ...
    deleteSelected(selectedRows: any[]) {
         ...
         this._inspectionReq.deleteInspectionReqOrder(row.Id)
       );
       // نفّذ كل الـ requests مرة واحدة
       forkJoin(deleteRequests)
         .pipe(
           finalize(() => this.inspectionReqOrderList()) // بعد الحذف رجّع القائمة
         )
         .subscribe({
           next: () => {
             this._toastr.success("Inspection Service Order deleted successfully");
             this.selectedRow = [];
             this.dataList.clearSelection();
             this.cd.detectChanges();
           },
           error: (err) => {
             this._toastr.error("Error deleting Inspection Service Order");
           }
         });
     }
